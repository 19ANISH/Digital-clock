...
if __name__=='__main__':
    window=tkinter.Tk()
    frame=tkinter.Frame(window, width=1000, height=1000)
    frame.pack()

    tkinter.Label(frame, text="Current time: ").pack()

    text=scrolledtext.ScrolledText(frame, height=15)
    text.pack()
    ...
    b2=tkinter.Button(frame, text="Current time",
                      command=lambda :text.insert("end",time.strftime("%I:%M:%S")+'\n'))
